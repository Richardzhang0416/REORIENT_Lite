...
function H_excl_end(slst,slln)
    ...
    en=collect(values(slst))+collect(values(slln)).-2
    out=reduce(vcat,collect(collect(st[i]:en[i]) for i in 1:length(values(pp.st))))
...
function dropComments(df)
    ...
    validRows=setdiff(1:size(df,1),listComments)
...
function transformLargeStructure!(obj,nameV);
...
function writeUNC(linkingTable,nameV,n2i,i2n)
    ...
    rRowLength=findlast([linkingTable[i][11] for i in eachindex(linkingTable)].=="r");
    hRowLength=findlast([linkingTable[i][11] for i in eachindex(linkingTable)].=="h")-rRowLength;
    cRowLength=findlast([linkingTable[i][11] for i in eachindex(linkingTable)].=="c")-(rRowLength+hRowLength);
    ###
...
function setSP!(b,lt)
    ...
    for i in rh
        if abs(lT[i][10])==1
            ...
            b.temp.x[lT[i][2],1] = (lT[i][12]/lT[i][10])*10;
        end
    end
...
function remove_noise(a,eps)
    ...
    if typeof(a)==Matrix{Float64} || typeof(a)==Vector{Float64}
